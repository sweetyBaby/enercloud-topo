...
文本框/变量/占位点**不参与连线避障**——可紧贴连线、设备任意摆放，直线不会因此变折线；挪动**游离**（未被任何连线引用）的标注元素也不触发全局重路由（消除"挪个文本，别的线跟着变形"的抖动）。占位点作为连线端点/汇合点时不受影响（避障本就排除连线自身的 from/to）。

> 实现：`topology-runtime.js` 的 `isRouteObstacle(n)` 统一判定（`text`/`variable`/`anchor` 返回 false），作用于 `pathHitsNodes`、`buildObstacleGrid`、`detourRoute` 三处避障，以及 `topoSig`（游离标注不计入拓扑签名）。

#### 占位点 ⇢ 汇合点（T 型分接 · 合并 · 汇合规整）

占位点（anchor）可作为连线的**汇合/分接点**，把多条线并到同一处：

- **落线分接**：占位点拖到连线上（`12/zoom` 容差）吸附到线上最近点并高亮（黄色环），松手即**分接**——原边 `A→B` 分裂为 `A→占位点`、`占位点→B` 两段。原边的 **id/标签/数据绑定留在前半段**（信号键 `edgeId.英文名` 不断链），后半段继承线型/流向/走线/线宽样式；手动折线的拐点按分接段拆给两半。从元素库新拖入占位点落在线上、连线右键「从此处引出连线」走同一逻辑。
- **占位点合并**：占位点拖到另一个占位点上（`12/zoom`，绿色虚线环）松手即**合并为一个**——被拖点的所有连线改接到目标点后删除被拖点；两点间的短线（合并后成自环）自动删除。
- **汇合规整（去重共用）**：任一提交点（新建连线/落点、拖动/合并占位点、拐点拖拽、端点重连）后自动运行 `normalizeAnchorJunctions()`——连线路径**穿过**某占位点时在该点分接；分接出的段落若与既有连线端点重复则**去重共用**（不产生叠线/重复箭头）。因此多条线汇合后下游只有**一条线、一个汇合点、一个箭头**。
- **去重的元数据保全**：分接/合并去重时优先保留带 **id/标签绑定/`showWhen`/`dirRules`** 的连线；双方都带元数据的重复边保留先出现的，并**明确告警**被删连线的信号键（提示可撤销），不静默丢链路。

**视觉呈现**：≥2 条连线接入的占位点（已成汇合点）自动**隐藏自身填充圆**，汇合点由 `drawJunctionDots` 画的线色实心点统一表达——画面上只有一个点；进出汇合点的连线**不画箭头**（箭头戳在汇合圆点上是噪音，流向仍由流动动画表达）。连线端点落在占位点时锚定到其**几何中心**（多条线交于同一点，不贴盒边）。游离/单边占位点照常显示蓝色圆点，便于继续拖拽和连线。

> **对既有连线的影响**：规则引擎（`showWhen`/`iconRules`/`dirRules`）与信号键均按**边对象/边 id/节点 id** 工作，不按 from/to 对匹配。未被占位点操作触及的连线 from/to 完全不变；被分接的边其 id/标签/绑定随保留段迁移、`showWhen`/`dirRules` 复制到两段，规则与信号链路保持一致。
