...
let edgeMode=false,edgeFrom=null,edgeWaypoints=[],pendingET='ac_power',pendingRoute='smart',mouseWX=null,mouseWY=null,selectMode=false;
...
canvas.addEventListener('mousedown',e=>{
  ...
  if(edgeMode){
    if(n){
      if(!edgeFrom){edgeFrom=n.id;edgeWaypoints=[];document.getElementById('ehint').textContent='连线['+ET[pendingET].label+']：点空白处加拐点，点目标节点完成';}
      else if(edgeFrom!==n.id){
        if(!edges.find(e=>e.from===edgeFrom&&e.to===n.id||e.from===n.id&&e.to===edgeFrom)){
          snapshot();
          const newEdge={from:edgeFrom,to:n.id,et:pendingET,dir:'forward',route:pendingRoute,lbl:''};
          if(edgeWaypoints.length>0){newEdge.waypoints=edgeWaypoints.slice();newEdge.route='manual';simplifyWaypoints(newEdge);}
          edges.push(newEdge);snapshot();
        }
        edgeFrom=null;edgeWaypoints=[];document.getElementById('ehint').textContent='连线['+ET[pendingET].label+']：点击起始节点…';
      ...
      const last=edgeWaypoints.length>0?edgeWaypoints[edgeWaypoints.length-1]:(()=>{const f=nodes.find(z=>z.id===edgeFrom);return f?[f.x,f.y-nsz(f)*0.22]:[wx,wy];})();
      // L型：与上一点对齐，取偏移大的方向
  ...
  if(selEdge && !edgeMode){
    ...
    if(cn){
      ...
      dragWaypoint={e:selEdge,i:idx};canvas.style.cursor='grabbing';return;
    }
    ...
    if(selEdge._endHandles){ for(let hi=0;hi<selEdge._endHandles.length;hi++){ const h=selEdge._endHandles[hi];
      if(h&&Math.abs(wx-h[0])<8/zoom&&Math.abs(wy-h[1])<8/zoom){
        ...
        dragEndpoint={e:selEdge,which,orig:selEdge[which]};canvas.style.cursor='grabbing';return;
      } } }
  }
  if(n){
...
canvas.addEventListener('mousemove',e=>{
  ...
  if(dragEndpoint){
    ...
    const hv=nodeAt(mouseWX,mouseWY);
    const otherId=dragEndpoint.e[dragEndpoint.which==='from'?'to':'from'];
    const tgt=(hv&&hv.id!==otherId)?hv.id:dragEndpoint.orig;
    if(dragEndpoint.e[dragEndpoint.which]!==tgt){ dragEndpoint.e[dragEndpoint.which]=tgt; _pathCacheSig=''; }
...
canvas.addEventListener('mouseup',()=>{
  ...
  if(dragEndpoint){
    const _e=dragEndpoint.e, changed=_e[dragEndpoint.which]!==dragEndpoint.orig;
  ...
  if(dragWaypoint){const _e=dragWaypoint.e;dragWaypoint=null;alignGuides=[];simplifyWaypoints(_e);
    // 把存储拐点同步为「实际渲染后的正交拐点」：强制正交会让线在直角处转弯而非原始点，
    // 不同步就会残留偏离线条的孤立手柄、并越拖越多。同步后手柄恒在线上、抓取即命中。
    _pathCacheSig=''; const rp=edgePath(_e); if(rp&&rp.length>2){ _e.waypoints=rp.slice(1,-1).map(p=>p.slice()); simplifyWaypoints(_e); }
...
window.addEventListener('keydown',e=>{
  ...
  if(e.key==='Escape'){if(edgeFrom){edgeFrom=null;edgeWaypoints=[];document.getElementById('ehint').textContent='连线['+ET[pendingET].label+']：点击起始节点…';}}
...
function drawAll(){
  ...
  if(edgeMode&&edgeFrom){
    ...
    if(f){
      const fb=nodeBox(f);
      const pts=[[f.x,fb.cy],...edgeWaypoints.map(p=>p.slice())];
      if(mouseWX!=null){
        const last=pts[pts.length-1];
        let px=mouseWX,py=mouseWY;
        if(Math.abs(mouseWX-last[0])>Math.abs(mouseWY-last[1]))py=last[1];else px=last[0];
        pts.push([px,py]);
      }
      ctx.save();
...
function simplifyWaypoints(e){
  ...
  const start=anchorPoint(a, wps[0][0], wps[0][1]);
  const end=anchorPoint(b, wps[wps.length-1][0], wps[wps.length-1][1]);
...
function toggleSelectMode(){
  ...
  // 与连线模式互斥
  if(selectMode&&edgeMode){edgeMode=false;edgeFrom=null;document.getElementById('btn-edge').classList.remove('active');document.getElementById('ehint').style.display='none';document.getElementById('ebar').classList.remove('show');}
...
function tidyEdges(){
  ...
  applyTidyRouting();
  _pathCacheSig='';snapshot();
  flashHint('已整理连线：直线优先 · 遇障/交叉转最优L型 · 自动汇合（剩余交叉 '+_countCrossRaw()+'）');
}
...
function ctxConn(){if(!edgeMode){toggleEdgeMode();}edgeFrom=ctxTgt.id;document.getElementById('ehint').textContent='连线['+ET[pendingET].label+']：已选"'+ctxTgt.label+'"，点目标';document.getElementById('ctxmenu').style.display='none';}
...
function buildJSON(){
  ...
  if(signalValues&&Object.keys(signalValues).length) obj.sampleSignals=Object.assign({},signalValues);
...
// 汇总当前画布全部可用信号：节点字段(id.字段) + id.status + id.online + 自定义全局信号
...
// 构造求值上下文：静态默认值(节点字段dv/状态/在线=true/自定义样例) 叠加注入的样例值
...
function signalValueMeta(name){
  ...
  if(p.field==='online')return {kind:'bool'};
  if(p.field==='status'){
    const base=['在线','离线','待机','运行','充电','放电','故障','告警','备用','发电'];
    const n=nodes.find(x=>x.id===p.node);if(n&&n.status&&!base.includes(n.status))base.unshift(n.status);
  ...
  const n=nodes.find(x=>x.id===p.node);const f=n&&(n.data||[]).find(d=>d.key===p.field);
  if(f){
...
function fieldOptionsFor(node){
  ...
  opts.push({v:'status',t:'状态'});opts.push({v:'online',t:'在线'});
  return opts;
}
// 某行「值」的下拉建议（在线→true/false；状态→常见状态；数值→当前静态值）
function valSuggestFor(r){
  if(r.field==='online')return ['true','false'];
  if(r.field==='status'){const base=['在线','离线','待机','运行','充电','放电','故障','告警','备用','发电'];const n=nodes.find(x=>x.id===r.node);if(n&&n.status&&base.indexOf(n.status)<0)base.unshift(n.status);return base;}
  ...
  const n=nodes.find(x=>x.id===r.node);const f=n&&(n.data||[]).find(d=>d.key===r.field);return (f&&f.dv!=='')?[String(f.dv)]:[];
}
...
//   未提供的信号回退到画布静态值（节点字段 value / 状态 / 在线=true / topology.signals 样例）。
...
'  信号寻址：节点字段=“节点id.字段名(中文)”，另有“节点id.status”“节点id.online”，以及 signals 里的全局信号。\n'+
'  运行端用法：\n'+
'    import { resolveDynamic } from "./runtime.js";\n'+
...
'    实时数据键名 = 规则里的信号名：节点字段「节点id.字段名」、状态「节点id.status」、在线「节点id.online」、全局信号名；\n'+
...
function applyTidyRouting(){
  // 交给智能路由引擎统一计算：最短·避障·少交叉
  edges.forEach(e=>{ delete e.waypoints; delete e.orthoDir; if(e.route!=='manual'&&e.route!=='arc'&&e.route!=='line') e.route='smart'; });
  _pathCacheSig=''; recomputeAllPaths();
}
...
//   实时数据键名 = 规则里用的信号名：节点字段「节点id.字段名」、状态「节点id.status」、在线「节点id.online」、全局信号名。
// ══════════════════════════════════════════════════════════════
